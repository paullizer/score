...
test('browser review keeps exact citations, protects unsaved and accepted drafts, ignores stale reads, and supports mobile history', { timeout: 90000 }, async (t) => {
  ...
  try {
    ...
    let protection = await visible(page.getByRole('dialog', { name: 'Leave unsaved grade changes?', exact: true }))
    ...
    assert.equal(await page.getByRole('dialog', { name: 'Leave unsaved grade changes?', exact: true }).count(), 0, 'closing an untouched nested quotation does not discard the outer draft')
    await page.evaluate(() => history.back())
    protection = await visible(page.getByRole('dialog', { name: 'Leave unsaved grade changes?', exact: true }))
    ...
    await editor.getByRole('button', { name: 'Close draft', exact: true }).click()
    protection = await visible(page.getByRole('dialog', { name: 'Wait for the grade request', exact: true }))
