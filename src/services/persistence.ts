...
import type { AnalysisRun, Job, Resume, Rubric, SourceDocument, Workspace } from '../domain/types'
import { validateRubric, weightedScore } from './scoring'
...
function recoverInterrupted(workspace: Workspace): Workspace {
  return {
    ...workspace,
...
export function loadWorkspace(): { workspace: Workspace | null; error: string | null } {
  ...
  if (!parsed.success) {
    ...
    return invalidSavedData(`${issue.path.map(String).join('.') || 'Workspace'}: ${issue.message}`)
  }
  const errors = referenceErrors(parsed.data)
  if (errors.length) return invalidSavedData(errors[0])
  return { workspace: recoverInterrupted(parsed.data), error: null }
}
...
export function saveWorkspace(workspace: Workspace): void {
  ...
  if (!parsed.success) {
    const issue = parsed.error.issues[0]
    throw new Error(`Cannot save invalid Score demo data at ${issue.path.map(String).join('.') || 'Workspace'}: ${issue.message}`)
  }
  const errors = referenceErrors(parsed.data)
  if (errors.length) throw new Error(`Cannot save inconsistent Score demo data: ${errors[0]}`)
  storage().setItem(WORKSPACE_STORAGE_KEY, JSON.stringify(parsed.data))
}
