...
test('worker results are matched to the request, validated, and terminated before returning bytes', async () => {
  ...
  const bytes = await api.generateReportInWorker(report, 'csv', { signal: controller.signal, onProgress: (message) => messages.push(message) })
...
test('foreign, malformed, and wrong-file worker responses fail closed', async () => {
  for (const reply of [
    ...
    await assert.rejects(api.generateReportInWorker(report, 'csv', { signal: new AbortController().signal }), /different|invalid/)
...
test('worker errors are visible and cancellation terminates generation without downloading', async () => {
  ...
  await assert.rejects(api.generateReportInWorker(report, 'csv', { signal: new AbortController().signal }), /cannot be rendered/)
  behavior = () => {}
  const controller = new AbortController()
  const pending = api.generateReportInWorker(report, 'csv', { signal: controller.signal })
  controller.abort()
  await assert.rejects(pending, (error) => error.name === 'AbortError')
  assert.equal(workers.at(-1).terminated, 1)
  assert.equal(downloads.length, 0)
  const before = workers.length
  await assert.rejects(api.generateReportInWorker(report, 'csv', { signal: controller.signal }), (error) => error.name === 'AbortError')
...
test('PDF font bytes come only from same-origin bundled assets and transfer into the worker', async () => {
  ...
  await api.generateReportInWorker(report, 'pdf', { signal: new AbortController().signal })
...
test('HTML, oversized, and failed font responses never start a PDF worker', async () => {
  for (const response of [
    ...
    await assert.rejects(api.generateReportInWorker(report, 'pdf', { signal: new AbortController().signal }), /font|TrueType/)
  }
...
test('one failed font request cancels other pending font work', async () => {
  ...
  await assert.rejects(api.generateReportInWorker(report, 'pdf', { signal: new AbortController().signal }), /fonts could not be loaded/)
...
test('download uses the expected MIME, safe filename and short-lived object URL without storing report data', async () => {
  ...
  assert.equal(filename, '-Private- review - partial.csv')
