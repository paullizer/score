...
self.addEventListener('message', (event: MessageEvent<ReportWorkerRequest>) => {
  ...
  void (async () => {
    ...
    request.report.capture.settings = captureReportSettings(request.report.capture.settings)
    ...
    if (!(output instanceof Uint8Array) || !output.byteLength || output.byteLength > limits.maxOutputBytes) {
    ...
    if (Date.now() - startedAt > limits.maxGenerationMilliseconds) throw new Error('Report generation exceeded its time limit. No file was generated.')
