...
function sample() { return foundation.api.buildSampleAnalysisReport(foundation.api.createInitialWorkspace().runs[0]) }
function first(report) { return report.groups[0].comparisons[0] }

test('real review links use the originating workspace and saved-result route with explicit real mode', () => {
  const report = real()
  const links = api.reportReviewLinks(report, first(report), { links: { origin: 'https://score.test/' } })
  assert.equal(links.analysis, 'https://score.test/workspaces/workspace-one/analyses/run-one?data=real&result=comparison-0')
...
test('sample links distinguish standalone samples from samples in an originating cloud workspace', () => {
  const report = sample(), pair = first(report)
  assert.equal(report.workspaceId, undefined)
  const standalone = api.reportReviewLinks(report, pair, { links: { origin: 'http://localhost:5173' } })
  const cloud = api.reportReviewLinks(report, pair, { links: { origin: 'https://score.test', workspaceId: 'cloud-workspace' } })
  for (const kind of ['analysis', 'resume', 'target']) {
    const standaloneUrl = new URL(standalone[kind]), cloudUrl = new URL(cloud[kind])
    assert.equal(standaloneUrl.pathname, `/analyses/${encodeURIComponent(report.run.id)}`)
    assert.equal(cloudUrl.pathname, `/workspaces/cloud-workspace/analyses/${encodeURIComponent(report.run.id)}`)
    for (const url of [standaloneUrl, cloudUrl]) {
      assert.equal(url.searchParams.get('data'), 'samples')
      assert.equal(url.searchParams.get('result'), pair.id)
      assert.equal(url.searchParams.get('view'), kind === 'analysis' ? null : kind)
    }
  }
})
...
test('workspace, run and result identities are encoded without changing the saved destination', () => {
  ...
  const url = new URL(api.reportReviewLinks(report, pair, { links: { origin: 'https://score.test:8443' } }).target)
  ...
  assert.equal(url.searchParams.get('data'), 'real')
...
test('missing or invalid origin configuration is explicit and never fabricates an application host', () => {
  ...
  ]) assert.throws(() => api.reportReviewLinks(report, pair, { links: { origin } }), /valid HTTP\(S\) application origin/, String(origin))
})

test('real workspace mismatches, missing identities, and unrelated comparisons fail closed', () => {
  const report = real(), pair = first(report), options = { links: { origin: 'https://score.test' } }
  ...
  assert.throws(() => api.reportReviewLinks(report, { ...pair, dataKind: 'sample' }, options), /comparison in this saved analysis/)
  for (const value of ['', '.', '..', '\u0000']) {
    assert.throws(() => api.reportReviewLinks({ ...report, run: { ...report.run, id: value } }, pair, options), /identity/)
    assert.throws(() => api.savedReviewPath({ workspaceId: value, runId: 'run', comparisonId: 'pair', dataKind: 'sample' }), /identity/)
  }
})
...
test('only one recognized source view on one selected result is honored', () => {
  ...
  assert.throws(() => api.savedReviewPath({ runId: 'run', comparisonId: 'pair', dataKind: 'real' }), /workspace/)
  assert.throws(() => api.savedReviewPath({ runId: 'run', comparisonId: 'pair', dataKind: 'sample', view: 'job' }), /resume or target/)
})
