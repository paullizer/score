...
export function DocumentViewer({ document, highlightedId, quote, compact = false, pagination = 'pdf-pages' }: {
  ...
  pagination?: 'pdf-pages' | 'html-sections' | 'captured-sections'
}) {
  ...
  return <div ref={container} className={`document-viewer ${compact ? 'document-compact' : ''}`}>
    ...
    <div className="document-scroll">
      {pages.map((page, pageIndex) => <article className="document-page" key={page} aria-label={`${document.title}, ${paginated ? 'page' : 'captured section'} ${page}`}>
        ...
        <footer><span>{document.sample ? 'Fictional document / Score demo' : 'Private workspace source'}</span><span>{paginated ? `Original page ${page} of ${pageCount}` : `${pagination === 'html-sections' ? 'Captured HTML' : 'Captured source'} section ${pageIndex + 1} of ${pages.length}`}</span></footer>
      </article>)}
    </div>
  </div>
}
