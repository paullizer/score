...
import { ArrowUpRight, Check, Loader2, Pencil, Plus } from 'lucide-react'
...
export function WorkspaceDirectory({ cloud, initialFilter = 'default', cards = false, disabled = false, showCreate = true,
  ...
  const busy = disabled || busyId !== null || creating
  ...
  useEffect(() => {
    onBusyChange?.(busyId !== null || creating)
    return () => onBusyChange?.(false)
  }, [busyId, creating, onBusyChange])
  ...
  return <section className="workspace-directory" aria-label="Workspace directory">
      ...
      <p className="mb-4 text-[11px] text-muted">{cards ? 'Counts include active real work only, never samples. ' : 'Owners can use the pencil beside a workspace to rename it. '}Search includes archived workspaces. Open one to read its content; unarchive to resume editing.</p>
      <ul className={cards ? 'workspace-home-grid' : 'workspace-switcher-list'}>
        {visible.map((item) => {
          ...
          return <li key={item.id} className={`${cards ? 'workspace-home-card' : 'workspace-switcher-item'} ${isCurrent ? 'is-current' : ''}`}>
            {cards && <div className="workspace-card-heading"><span className="workspace-monogram" aria-hidden="true">{item.name.trim().slice(0, 1).toUpperCase()}</span><Badge>{item.role}</Badge></div>}
            {isRenaming ? <form className="workspace-switcher-rename" onSubmit={(event) => { event.preventDefault(); void handleRename(item.id) }}>
              ...
              <Button size="sm" type="submit" variant="primary" icon={isBusy ? Loader2 : Check} disabled={busy || !renameValue.trim() || Boolean(item.archivedAt) || Boolean(incomplete)}>Save</Button>
      ...
      {!visible.length && <EmptyState title={query ? 'No matching workspaces' : !cloud.workspaces.length ? 'Your first workspace starts here' : filter === 'archived' ? 'No archived workspaces' : 'No active workspaces'}
        description={query ? 'Try a different workspace name or change the archive filter.' : 'Create a workspace to keep your jobs, resumes, and analyses together, or use the archive filter to find retained work.'} />}
      {showCreate && <div className="mt-4"><WorkspaceCreateButton cloud={cloud} disabled={disabled || busyId !== null} onBusyChange={setCreating} onCreated={onSelected} /></div>}
  </section>
}

export function WorkspaceCreateButton({ cloud, disabled = false, onBusyChange, onCreated, primary = false }: {
  cloud: Pick<WorkspaceDirectoryActions, 'createWorkspace'>
  disabled?: boolean
  onBusyChange?: (busy: boolean) => void
  onCreated?: () => void
  primary?: boolean
  ...
  const reason = policy.cloud && policy.phase !== 'ready' ? policy.error ?? 'Checking workspace creation policy.'
    : policy.settings?.workspaces.allowCreation === false ? 'Creating new workspaces is disabled by application policy.' : null
