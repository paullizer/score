...
function RunView({ run }: { run: AnalysisRun }) {
  ...
  return <>
    ...
    <PageHeader eyebrow="EVIDENCE-LED REVIEW" title={run.name} description="A clear view of the match, and the passages behind it."
      actions={<><EntityLifecycleActions target={{ kind: 'analysis', id: run.id }} name={run.name} onComplete={(action) => { if (action === 'delete') navigate('/analyses') }} />{working && <Button icon={X} disabled={!canEdit} onClick={() => cancelRun(run.id)}>Cancel pending</Button>}{needsRetry && !working && <Button icon={RotateCcw} disabled={!canEdit} onClick={() => retryRun(run.id)}>Retry unfinished</Button>}
        <Button icon={Sparkles} disabled={!canEdit} onClick={() => navigate(`/analyses/new?from=${run.id}`)}>New run with these inputs</Button></>} />
