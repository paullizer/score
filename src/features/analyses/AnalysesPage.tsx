      ...
      {runs.length ? <div className="table-wrap"><table className="data-table">
        ...
        <tbody>{runs.map((run) => {
          ...
          return <tr key={run.id}>
            <td><div className="flex min-w-[220px] items-center gap-3"><span className="job-monogram"><BarChart3 size={19} strokeWidth={1.5} /></span><div><Link to={sampleDataLink(`/analyses/${run.id}`, Boolean(cloud))} className="row-title">{run.name}</Link><div className="row-meta">{run.comparisons.length} separate comparisons / saved snapshots</div><div className="mt-1"><Badge tone="accent">Simulated scoring</Badge></div></div></div></td>
            ...
            <td><Link to={sampleDataLink(`/analyses/${run.id}`, Boolean(cloud))} className="button button-ghost icon-button" aria-label={`Open ${run.name}`}><ArrowUpRight size={16} /></Link></td>
          </tr>
        })}</tbody>
      </table></div> : <EmptyState icon={Clock3} title={workspace.runs.length ? 'No matching analyses' : 'A place for every comparison'} description={workspace.runs.length ? 'Try a different search or filter.' : 'Choose your resumes and criteria. The evidence will take it from there.'} action={<Button onClick={() => { if (!workspace.runs.length) navigate(sampleDataLink('/analyses/new', Boolean(cloud))); else { setQuery(''); setFilter('all') } }}>{workspace.runs.length ? 'Clear filters' : 'Create an analysis'}</Button>} />}
