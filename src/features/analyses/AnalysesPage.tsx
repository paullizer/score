...
function SampleAnalysesPage() {
  const { workspace, cloud } = useWorkspace()
  const navigate = useNavigate()
  const [query, setQuery] = useLibraryViewState('analyses:samples:query', '')
  const [filter, setFilter] = useLibraryViewState<'all' | 'complete' | 'attention'>('analyses:samples:status', 'all')
  const [archiveFilter, setArchiveFilter] = useLibraryViewState<ArchiveFilter>('analyses:samples:archive', 'default')
  const { canEdit } = useLifecycleAccess()
  const [sort, setSort] = useLibraryViewState<TableSort<SampleAnalysisSortKey> | null>('analyses:samples:sort', null)
    ...
    <section className="panel" aria-label="Analysis history">
      ...
      {runs.length ? <div className="table-wrap"><table className="data-table">
        ...
        <tbody>{runs.map((run) => {
          ...
          const name = getDisplayName(run, run.name)
          const jobs = run.targets.filter((target) => target.kind === 'job').length
          const grades = run.targets.length - jobs
          return <tr key={run.id}>
            <td><div className="flex min-w-[220px] items-center gap-3"><span className="job-monogram"><BarChart3 size={19} strokeWidth={1.5} /></span><div><Link to={sampleDataLink(`/analyses/${run.id}`, Boolean(cloud))} className="row-title">{name}</Link> <ArchivedBadge target={{ kind: 'analysis', id: run.id }} /><div className="row-meta">{run.comparisons.length} separate comparisons / saved snapshots</div><div className="mt-1"><Badge tone="accent">Simulated scoring</Badge></div></div></div></td>
            ...
            <td><div className="flex flex-wrap items-center gap-1"><Link to={sampleDataLink(`/analyses/${run.id}`, Boolean(cloud))} className="button button-ghost icon-button" aria-label={`Open ${name}`}><ArrowUpRight size={16} /></Link><RenameEntityButton target={{ kind: 'analysis', id: run.id }} name={name} compact /><EntityLifecycleActions target={{ kind: 'analysis', id: run.id }} name={name} compact /></div></td>
          </tr>
        })}</tbody>
