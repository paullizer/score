      ...
      {runs.length ? <div className="table-wrap"><table className="data-table">
        ...
        <tbody>{runs.map((run) => {
          ...
          return <tr key={run.id}>
            <td><div className="flex min-w-[220px] items-center gap-3"><span className="job-monogram"><BarChart3 size={19} strokeWidth={1.5} /></span><div><Link to={`/analyses/${run.id}`} className="row-title">{run.name}</Link><div className="row-meta">{run.comparisons.length} separate comparisons / saved snapshots</div><div className="mt-1"><Badge tone="accent">Simulated scoring</Badge></div></div></div></td>
