...
export function ApprovedGradeAnalysis({ ladderId }: { ladderId: string }) {
  ...
  if (!api) return null
  const targets = api.targets.state === 'ready' ? api.targets.value.filter((target) => target.kind === 'grade' && target.selection.ladderId === ladderId) : []
  ...
  return <section className="mt-4 space-y-3 border-t pt-4" aria-label="Approved GS analysis targets">
    ...
    {targets.map((target) => <div key={target.id} className="space-y-2 text-[11px]">
      ...
      {api.canWrite && available ? <Link className="text-link" {...realAnalysisLink({ targets: [target.selection] }, api.workspaceId)}>Analyze resumes with this approved version <ArrowRight size={13} aria-hidden="true" /></Link>
        : <p className="text-muted">Read-only or unavailable · an owner or editor can start a real analysis.</p>}
    </div>)}
    {targets.length > 1 && available && api.canWrite && <Link className="button button-secondary button-sm" {...realAnalysisLink({ targets: targets.map((target) => target.selection) }, api.workspaceId)}>Review all approved targets</Link>}
