...
export function GradeLadderLibrary({ search = '' }: { search?: string }) {
  ...
  if (!api) return null
  const visible = api.summaries.filter(({ ladder }) => `${ladder.name} ${ladder.context.series} ${ladder.context.agency} ${ladder.seedJobTitle} ${ladder.grades.map((grade) => `GS-${grade}`).join(' ')}`.toLowerCase().includes(search.trim().toLowerCase()))
  return <div className="grade-library">
    ...
    {visible.length > 0 && <div className="grade-family-grid">{visible.map(({ ladder, levels }) => <article className="grade-family-card" key={ladder.id}>
      <div className="flex flex-wrap items-center gap-2"><Layers3 size={17} className="text-accent" aria-hidden="true" /><Badge tone="accent">Real · Series {ladder.context.series}</Badge><Badge>{ladder.status.replaceAll('-', ' ')}</Badge></div>
      ...
      <ul className="grade-family-levels">{[...levels].sort((a, b) => a.head.grade - b.head.grade).map(({ head }) => <li key={head.id}>
