...
export function GradeLadderPage({ ladderId: givenId }: { ladderId?: string }) {
  ...
  useEffect(() => { if (ladderId) void ensure?.(ladderId) }, [ensure, ladderId])
...
function GradeLadderWorkspace({ detail, loadError }: { detail: GradeLadderDetail; loadError?: string }) {
  ...
  return <>
    ...
    <PageHeader eyebrow="REAL GS GRADE LADDER" title={detail.ladder.name} description={`Series ${detail.ladder.context.series} · ${detail.ladder.context.agency || 'Agency unresolved'} · Seed rubric v${detail.ladder.seedRubricVersion}`}
      actions={<><Button icon={Pencil} disabled={!api.canWrite || busy || sourceDirty} title={sourceDirty ? 'Finish source decisions first.' : !api.canWrite ? 'Viewer access is read-only.' : undefined} onClick={() => setContextEditor(true)}>Context / add grades</Button>
    ...
    <div className="detail-metadata"><Badge tone="accent">Real · private server records</Badge><Badge>{detail.ladder.status.replaceAll('-', ' ')}</Badge><span>{detail.ladder.grades.map((item) => `GS-${item}`).join(' / ')}</span><span>{detail.ladder.context.supervision}</span><span>Seed: {detail.ladder.seedJobTitle}</span></div>
    ...
    {view === 'matrix' && <GradeMatrix detail={detail} selectedGrade={grade} onGrade={setGrade} canWrite={api.canWrite} pending={busy} unsavedSources={sourceDirty} onSource={setSource} onEdit={setDraft} onHistory={openHistory} onApprove={approve} />}
    ...
    <section className="panel grade-work-items"><div className="section-heading"><div><h2>Durable processing progress</h2><p>Background work survives browser close after request acknowledgement. Failures are separate from evidence gaps.</p></div>
      <Button size="sm" icon={RotateCcw} disabled={!api.canWrite || busy || sourceDirty || active} title={sourceDirty ? 'Finish source decisions first.' : active ? 'Wait for or cancel current work before rediscovery.' : 'Discover a new source proposal without overwriting historical captures.'}
      ...
      <div className="grade-work-list">{detail.workItems.length ? [...detail.workItems].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map((work) => <article key={work.id}>
        ...
        <div className="flex flex-wrap gap-2"><Badge tone={work.status === 'failed' ? 'danger' : work.status === 'succeeded' ? 'success' : 'neutral'} dot>{work.status}</Badge>
          {['failed', 'cancelled'].includes(work.status) && <Button size="sm" disabled={!api.canWrite || busy || sourceDirty || work.error?.retryable === false} title={work.error?.retryable === false ? 'This failure is not retryable. Correct sources or context before starting new work.' : undefined}
          ...
          {['queued', 'running'].includes(work.status) && <Button size="sm" variant="ghost" icon={X} disabled={!api.canWrite || busy || sourceDirty} onClick={() => execute(() => api.cancel(detail.ladder.id, { workId: work.id }, detail.etag), 'Cancellation requested. Published versions and completed source captures remain retained.')}>Cancel stage</Button>}</div>
