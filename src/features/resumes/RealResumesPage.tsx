    ...
    <section className="panel" aria-label="Real resume library">
      ...
      {visible.length ? <div className="table-wrap"><table className="data-table">
        ...
        <tbody>{visible.map((summary) => {
          ...
          return <tr key={summary.resume.id} className={checked ? 'row-selected' : ''}>
            ...
            <td className="min-w-[230px] max-w-[440px]"><RealResumeStatus summary={summary} />
              ...
              <p className="mt-1 text-[10px] text-muted">{summary.source.kind === 'markdown' ? 'Markdown' : summary.source.kind === 'pdf' ? 'PDF' : 'Public URL'} · added {dateLabel(summary.resume.createdAt)} · attempt {summary.attempts}</p>
        ...
        description={api.phase === 'loading' ? 'Loading every page of authorized resume summaries.' : api.phase === 'error' ? 'No samples are substituted. Retry the service when available.'
          : search ? 'Try a stated name, role, or source label. Hidden selections are retained.' : 'Choose actual PDFs, local Markdown files, or public URLs. Inaccessible inputs get individual errors; successful imports remain available.'}
  ...
  return <>{back}
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <section className="detail-panel" aria-label="Actual resume source">
        ...
        {detail.document ? <DocumentViewer document={detail.document} pagination={detail.extraction?.pagination ?? documentPagination(detail.capture?.original.contentType)} />
        ...
        <div className="source-footer"><span className="break-all">{detail.source.displayName}</span><span>{detail.source.kind === 'markdown' ? 'Uploaded Markdown' : detail.source.kind === 'pdf' ? 'Uploaded PDF' : 'Public source capture'}</span></div>
      </section>
      <aside className="space-y-5" aria-label="Resume profile and capture provenance">
        ...
        <section className="detail-panel"><div className="section-heading"><h2>Capture provenance</h2></div>
          <div className="space-y-4 p-5 text-[11px]">
            ...
            {detail.extraction && <p>{detail.extraction.method} · parser {detail.extraction.version} · {detail.extraction.pagination === 'pdf-pages' ? `${detail.extraction.pageCount ?? 'Unknown'} PDF pages` : detail.extraction.pagination === 'markdown-sections' ? 'Markdown sections, not PDF pages' : 'Captured HTML sections, not PDF pages'}</p>}
