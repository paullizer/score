...
export function RubricPanel({ rubric, onSelectCriterion, onVersionSaved, readOnly = false }: {
  ...
  const { workspace, saveRubric } = useWorkspace()
  ...
  const editable = rubric.kind === 'grade' || job?.status === 'ready'
  ...
  function duplicate() {
    if (duplicatingRef.current || rubric.kind !== 'grade') return
    duplicatingRef.current = true
    setDuplicating(true)
    setError('')
    try {
      const id = saveRubric({ ...structuredClone(rubric), name: `${rubric.name} (copy)` }, true)
      onVersionSaved?.(id)
    } catch (caught) {
    ...
    <div className="space-y-5 p-5">
      <div>
        <div className="mb-3 flex flex-wrap gap-2">
          <Badge tone="accent">{rubric.kind === 'job' ? 'Generated demo' : 'Reusable grade rubric'}</Badge>
      ...
      <div className="rounded-xl border bg-soft p-3.5">
        <div className="flex items-start gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-medium">{rubric.kind === 'job' ? 'Grounded in the sample job document' : 'A standard you can use without a job'}</p>
      ...
      {!readOnly && <div className="flex flex-wrap gap-2">
        ...
        {rubric.kind === 'grade' && <Button icon={Copy} size="sm" variant="ghost" onClick={duplicate} disabled={duplicating}>
          {duplicating ? 'Duplicating…' : 'Duplicate as new rubric'}
        </Button>}
      </div>}
      {!readOnly && !editable && <p className="text-[11px] text-muted">Finish the linked job import before editing this rubric.</p>}
      ...
      <div>
        ...
        {rubric.criteria.map((criterion, index) => {
          const source = document?.paragraphs.find((paragraph) => paragraph.id === criterion.sourceParagraphId)
          return <article className="criterion-card" key={criterion.id}>
            ...
            {criterion.key === 'custom' && <div className="mt-3">
      ...
      <DemoNote>Demo scoring is deterministic, not an AI prediction. Review the source evidence rather than treating a score as a hiring decision.</DemoNote>
    </div>
