...
test('100 resumes / 500 comparisons use constant candidate reads, no comparison narrative fan-out, and four ordered full-summary readers', async t => {
  ...
  assert.equal(observed.calls.get.length, 5)
  ...
  assert.equal(observed.calls.list.length, 2)
  assert.ok(observed.calls.list.every(options => options.targetId === pair.target.summary.id))
  assert.deepEqual(observed.calls.list.map(options => options.recordType), ['analysis-comparison', 'analysis-candidate-narrative'])
...
test('candidate reads ignore missing unrelated publications and retain pending generations and prior text without repairing metadata', async () => {
  ...
  try {
    ...
    assert.equal(observed.calls.get.length, 6, 'Pending requests add just one receipt point read.')
