  ...
  router.post(
    ...
    mutation('write', async (req, res) => {
      ...
      if (existing) {
        ...
        res.json({ job: await summary(jobs.store, existing) })
        return
      }
      const sourceBlob = await putJobBlob(jobs.store, jobs.blobs, workspaceId, jobId, blobName, bytes, 'application/pdf')
