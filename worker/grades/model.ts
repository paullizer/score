...
Missing requirement-source support must remain an explicit support gap with blocking issues until applicable sources are added. This is different from a later assessment of a successfully reviewed usable resume with no supporting evidence for an established professional criterion, which is a document-evidence score of zero. There is NO custom-expectation, user-confirmation, or model-supported override for missing requirement-source support. A source or version global blocker remains blocking; a grade-specific issue affects only that grade.
