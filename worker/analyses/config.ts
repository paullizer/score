...
export interface AnalysisWorkerConfig extends WorkerModelConfig {
...
function required(environment: NodeJS.ProcessEnv, key: WorkerConfigurationField): string {
  ...
  if (!value) throw new WorkerConfigurationError(key, 'missing', `${key} is required for the analysis worker.`)
  return value
}
...
export function loadAnalysisWorkerConfig(environment: NodeJS.ProcessEnv): AnalysisWorkerConfig {
  ...
  if (corrections && corrections !== 'true' && corrections !== 'false') {
    throw new WorkerConfigurationError('ANALYSIS_EVIDENCE_CORRECTIONS_ENABLED', 'invalid-value', 'ANALYSIS_EVIDENCE_CORRECTIONS_ENABLED must be true or false.')
  }
  ...
  if (localDevelopment && (environment.NODE_ENV === 'production' || environment.IDENTITY_ENDPOINT ||
    ...
    throw new WorkerConfigurationError('WORKER_AUTH_MODE', 'hosted-auth', 'Azure CLI analysis-worker authentication is not allowed in a hosted runtime.')
  }
  ...
  if (container !== 'analysis-records' || blobContainer !== 'analysis-sources') {
    throw new WorkerConfigurationError(container !== 'analysis-records' ? 'ANALYSIS_RECORDS_CONTAINER' : 'ANALYSIS_SOURCE_CONTAINER',
      'store-isolation', 'The analysis worker must use its dedicated analysis-records and analysis-sources stores.')
  }
  const forbidden = ([
  ...
  ] as const).find(key => environment[key]?.trim())
  if (forbidden) {
    throw new WorkerConfigurationError(forbidden, 'store-isolation', 'Live job, grade, resume, legacy workspace, OCR, and renderer services must not be configured in the analysis worker.')
  }
