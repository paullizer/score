...
**Authenticated cloud mode supports real job and resume imports, source-grounded GS ladders, and manually requested evidence-backed analyses.** Microsoft Entra authentication and workspace membership protect private sources and results. Each real feature requires its configured stores and deployed worker; this repository does not imply that an existing Azure environment has been upgraded. Scores are document-evidence review aids, not official GS eligibility findings, intrinsic measures of a person, or hiring decisions.

## Architecture

![Score architecture: React browser, Entra-protected API, Cosmos and Blob storage, four background workers, source extraction, and Foundry inference.](docs/architecture.svg)
...
The diagram describes the base ingestion/analysis architecture, not the verified state of a live deployment. **QC adds a fifth isolated worker**, private `qc-records` / `qc-sources` stores, and immutable prompt versions in the application-settings container. **The optional rubric AI assistant**, not drawn here, adds one direct, synchronous API-to-Foundry call per reviewer request. It stores nothing ([AI-assisted editing](docs/assisted-editing.md)). Cosmos feature records also form the durable work queue; there is no separate message broker. Each worker uses its own feature stores, and explicitly requested analyses consume frozen inputs. The QC worker sees only explicitly captured case packs, cannot publish production scores, and cannot change the application-wide prompt pointer. Report files are generated by a browser Web Worker, not the hosted Chromium renderer. Private evidence is never indexed into shared Foundry IQ or AI Search.

## Run locally

Use Node.js 24 and npm. `npm run dev` serves the authenticated app and proxies `/api` to a locally running Score API. There is no offline demo: without a reachable API, the app shows its cloud-service-unavailable screen.

Vite proxies `/api` for both `npm run dev` and `npm run preview` to `SCORE_API_ORIGIN`, defaulting to `http://127.0.0.1:8080`. The Node-side proxy always strips any browser-supplied `X-Score-Dev-Principal` header; when `SCORE_DEV_PRINCIPAL` is set, it replaces that header with the configured value on proxied requests. That value is never exposed to the browser bundle.
...
Run the API in one terminal. The API uses your Azure CLI login locally via `AzureCliCredential`, never account keys, so sign in with an account that has data-plane access to the configured Cosmos DB and Storage accounts. `SCORE_AUTH_MODE=dev-header` binds the API to 127.0.0.1 only.

```powershell
az login
$env:SCORE_AUTH_MODE = 'dev-header'
$env:AZURE_TENANT_ID = '<tenant-id>'
$env:SCORE_DEV_USER_ROLES = '{"<your-object-id>":["Score.User","Score.Admin"]}'
...
# Optional real features; each needs its stores, and queued work needs its deployed worker:
# $env:REAL_JOB_IMPORTS_ENABLED = 'true'; $env:JOB_RECORDS_CONTAINER = 'job-records'; $env:JOB_SOURCE_CONTAINER = 'job-sources'
# $env:REAL_RESUME_IMPORTS_ENABLED = 'true'; $env:REAL_GRADE_LADDERS_ENABLED = 'true'; $env:REAL_ANALYSES_ENABLED = 'true'
npm run build:server
node dist-server/server.mjs
```

See the configuration and feature-flag sections below for additional environment names and defaults, including `COSMOS_DATABASE=score`, `COSMOS_CONTAINER=workspaces`, `WORKSPACE_BLOB_CONTAINER=workspace-state`, and `PORT=8080`.

`APP_ORIGIN` must exactly equal the Vite origin. The API CSRF middleware requires the browser `Origin` to match `APP_ORIGIN` on POST/PUT/PATCH/DELETE, along with the app-sent `X-Score-Request: workspace` header. In Easy Auth mode `APP_ORIGIN` must be HTTPS; in `dev-header` mode only, HTTP loopback origins (`http://127.0.0.1:<port>`, `http://localhost:<port>`, `http://[::1]:<port>`) are accepted. If Vite chooses another port, update `APP_ORIGIN`; `npm run preview` defaults to port 4173.

Run the browser app in a second terminal:

```powershell
$env:SCORE_DEV_PRINCIPAL = '<tenant-id>:<your-object-id>'
# $env:SCORE_API_ORIGIN = 'http://127.0.0.1:8080'  # only if the API runs elsewhere
npm run dev
```

Open `http://127.0.0.1:5173`. `/.auth/login` and `/.auth/logout` are App Service Easy Auth endpoints and are not available locally, so Sign in / Sign out do not work. Switch identity by restarting Vite with another `SCORE_DEV_PRINCIPAL` that is configured in `SCORE_DEV_USER_ROLES`.
...
Start at **My workspaces**. Choose a workspace, reopen one from **Recent workspaces**, or, when authorized, use **New workspace** to give a new one a name. Owners and application Admins can manage sharing from the home cards or workspace picker. Cards show active jobs, resumes, and analysis runs; archived content is excluded. Counts load independently; an unavailable count is not zero and does not prevent opening the workspace. Use **All workspaces** in desktop/mobile navigation or the application logo to return home. Opening a saved workspace or record link still takes you directly there.

Inside a workspace, the sidebar lists **Jobs**, **Resumes**, **Rubrics**, and **Analyses** with their active counts. Its bottom group holds **Application settings** and **Users / user access** (application Admins only), **About Score** (named for the configured application title), and the **Support** and **Documentation** links when an administrator sets them, followed by Appearance and your account. **Collapse navigation** shrinks the sidebar to an icon rail that keeps every item, with a tooltip naming each one; **Expand navigation** restores it. This browser remembers the choice. On short screens, the navigation scrolls while Appearance and your account stay in view. Below 900px wide, the menu button opens the same navigation as a drawer, which is never collapsed.

1. **Jobs:** Choose **Add jobs** to upload actual PDFs or advertised Markdown files (`.md` / `.markdown`), plus DOCX and legacy DOC when Word imports are enabled, or import direct public posting URLs. Each input creates its own job and associated rubric. Listing pages and whole-site discovery are not imported.
2. **Resumes:** Upload actual PDFs, advertised Markdown files, or enabled Word formats, or enter public resume/profile URLs, one per line, with up to 10 total inputs per batch. Inspect ready documents, provenance, and item-level errors.
3. **Rubrics:** Review job-specific criteria or choose **Create grade ladder** from a job to prepare separate GS levels. Inspect the selected sources and grade matrix before approving supported versions. Weights must total 100. When the deployment enables it, **Edit with AI** opens a job rubric with a scoped assistant beside the job posting. Its changes are applied to your unsaved draft, highlighted until you save, and undoable; see [AI-assisted editing](docs/assisted-editing.md).
4. **Analyses:** Select ready resumes and saved job rubrics or exact approved GS versions, review the comparison count, then explicitly start the run. The limit is 500 resume/target pairs. Importing never starts scoring automatically.
...
When summaries are generated automatically, a saved analysis isn't **Complete** until its scoring and its summaries have both finished, because PDF, Word, and PowerPoint reports need the summaries. Each comparison moves through **Queued for analysis**, **Analyzing**, **Analysis complete**, **Queued for summary generation**, and **Generating summary** to **Complete**; failures, cancellations, and re-scores keep their own labels. The analysis page shows one progress bar: a comparison counts once, when its score and its candidate summary are both done, and each job or grade overview counts once. Below the bar, separate lines count analysis, candidate summaries, and overviews. A failed, missing, or out-of-date summary still counts as finished work, but the analysis shows **Summaries need attention** until **Manage summaries** fills it in. Analyses whose summaries are generated on demand, or that started while summary generation was off, show **Complete** once scoring finishes, with a note that PDF, Word, and PowerPoint need their summaries first. In the analysis list, a **Summaries** line shows how many summaries are ready, queued, generating, or need attention, and the **Complete** filter and status sorting apply the same rule. Progress comes from a lightweight status read that never loads summary text; if it fails, the page keeps the last known progress and offers **Check summary progress**.
...
Imports and analyses show durable server progress and actual processing errors. One inaccessible URL or failed comparison does not discard successful items. Real analyses evaluate the exact saved criteria, including custom wording, against the frozen resume evidence.

Source-backed grade drafts and real assessments require human review and are not official OPM classification or eligibility determinations. Missing resume evidence does not prove a person lacks a skill. Do not treat a score as a hiring recommendation or automate employment decisions from it.
...
4. Foundry generates a schema-constrained rubric from the extracted source. Criteria include required/preferred distinctions, 0-5 scoring guidance, weights totaling 100, and exact paragraph quotations. Metadata, weights, and citations are validated before publication; invalid output is repaired once or reported as an error.
...
Real jobs live in Cosmos `job-records`, partitioned by workspace. These records are also the durable work queue; there is no separate queue publication that can get out of sync. Originals and normalized source documents live in `job-sources`. They are independent of workspace directory metadata and legacy workspace state blobs.

The dedicated worker identity can access job records/sources, Document Intelligence, and Foundry inference, but not the workspace-state Blob container used for mutation leases and legacy state, or shared knowledge contents. Public HTTP requests use DNS-pinned connections, validate every redirect, and reject private, loopback, metadata, and other reserved addresses.
...
**The rubric AI assistant is the one synchronous exception.** It is on by default wherever real jobs and the job-rubric model deployment are configured, and administrators turn it off under **Admin settings → Features & intake → Rubric AI assistant**. Each assistant turn sends the job's extracted text, the reviewer's unsaved draft, and the instruction **from the API identity** to the same `jobRubric` deployment and US data zone. The API identity already holds the model-user role it uses for administrator model probes. The assistant writes nothing and stores no conversation, and its telemetry is content-free. Assisted edits must still quote the posting exactly and are saved only through the normal rubric save. See [AI-assisted editing](docs/assisted-editing.md).

### Uploaded formats, evidence, and private Word previews

Markdown and Word support applies only to **job-description and resume file uploads**. It does not add Markdown/Word URL ingestion or permit independent agency/OPM reference Markdown/Word uploads. A ready Markdown- or Word-backed job can still seed a GS ladder, and those ready jobs/resumes can participate in explicitly requested analyses.
...
Real ladder families, work items, heads, versions, reviews, and approvals are stored in private Cosmos `grade-records`. Originals and extracted reference snapshots are stored in private Blob `grade-sources`, not shared IQ. The separate scheduled grade worker has its own identity and no access to job or legacy workspace stores. It shares the existing model, OCR service, and internal renderer, so normal throttling/backoff still applies.
...
Scoring is a separate, **manual** action after import. Choose ready resumes and job targets or approved GS rubric versions. The server resolves authorized IDs and exact versions, rejects stale, unsupported, or inconsistent selections, and freezes the complete comparison inputs. A newer unapproved GS draft does not replace the selected approved version. The UI shows the resume/target pair count before submission; more than **500 comparisons** is rejected, never silently truncated. For example, 103 resumes against four jobs produce 412 comparisons in one run.

Starting a run freezes a copy of every selected resume and target before the server answers. Score freezes them in parallel batches of eight, so 103 resumes against four jobs start in seconds rather than about a minute. While it works, the page shows **Starting analysis…** and says that large analyses can take a minute or two. The browser waits up to two minutes for each try, inside App Service's 230-second request limit. If a try times out, the connection drops, or the workspace is busy with another change, the browser quietly sends the same request again with the same request key, for up to 10 minutes in total. The server either returns the run it already accepted or accepts the request once, so re-sending never starts a second copy or picks up newer input versions. A definite answer, such as a stale selection or new work being paused, is shown straight away. If 10 minutes pass without any answer, the page says the analysis may still be starting. Either way it then offers **Try again**, which sends the same request, and **Change selections**.
...
### Optional QC mode and quality improvement

**QC is a separate cloud-only review workspace, not a required step in ordinary analyses.** Enter it with **QC mode** in the top bar and leave it with **Normal mode** in the same place; in QC, the sidebar lists only **Reviews**, **Quality improvement**, and **Prompt versions**. Workspace reviewers, editors, owners, and application admins with explicit workspace membership can review saved comparisons. Reviewer membership does not permit importing, rescoring, editing rubrics, managing sources, or other ordinary workspace writes. An Owner or application Admin can choose an eligible Entra user and the **Reviewer** role in **Manage access**. Eligibility requires the `Score.User` or `Score.Admin` application role; granting membership does not invite an external account or change application-role assignments. Application Admins retain tenant-wide ordinary workspace access, but QC always requires a valid explicit membership. Existing original-download and report-export role policies remain independent.

Each person keeps their own draft and immutable submitted revisions for an exact comparison result revision and hash. For every criterion, choose **Agree**, **Disagree**, or **Unable to judge**. Disagreements require a reason and proposed 0-5 rating or explicit unassessed/not-applicable outcome; inability to judge requires a reason. Submission requires a response to every criterion in that comparison, not every comparison in the run. There is no required assignment, review count, quorum, or schedule. Optional batches organize occasional review exercises. Feedback never changes the saved production assessment.

Peer feedback stays hidden until the reviewer submits their own review of that exact result. Owners, editors, and member application admins may explicitly aggregate feedback earlier; that exposure is recorded, and later edits cannot claim to be independent. Review history and planning use the same visibility rules. Multiple opinions remain separate; the most popular rating is not automatically declared correct, and repeated submissions by one reviewer do not become extra votes.

Newly accepted full assessments capture criterion-level confidence and ambiguity alongside the original final assessment call. QC displays this saved, result-bound information without making another diagnostic model request. It is model-reported certainty about applying the rubric, not a probability of correctness or a judgment of personal capability. These details never hold up a score. Score fixes obvious formatting slips, such as listing the chosen rating as an alternative. When a criterion's details still don't follow the diagnostic rules, for example alternative ratings with no ambiguity explanation, QC shows **Not recorded** for that criterion and the score is published anyway. Historical results without captured diagnostics also display **Not recorded**. A later scoped score correction does not manufacture fresh confidence for unchanged criteria.

Within QC, **Quality improvement** lets an authorized reviewer curate up to 25 cases and 100 permitted submitted reviews into a frozen, workspace-local case pack. Include or exclude reviews explicitly; distinguish drafting cases from holdouts and optionally record attributed reference decisions with rationale. AI drafts findings, disagreements, proposed generalized guidance, expected effects, and risks. Humans edit that proposal before requesting a paid baseline/candidate trial. Held-out human labels are excluded from planner and trial model inputs; rubric changes that alter criterion meaning cannot masquerade as like-for-like rating improvements.

Editable guidance is limited to job-rubric generation, GS competency generation, GS draft generation, and assessment. Fixed scoring, evidence, privacy, and grounding contracts are separately pinned, not editable by a plan. Only an **application admin with explicit workspace membership** can activate an evaluated candidate, with an explicit reason and exact version checks. Activation and rollback append immutable prompt history rather than overwriting old versions. Approved guidance applies app-wide only to newly accepted work; ordinary retries keep their captured prompt/model/settings versions, and only an explicit **Retry with current rules** or **Re-score with the current rules** request adopts newer ones. New rubric guidance affects future rubric creation, never existing saved rubrics or historical scores. Private reviewer comments, names, and quotations must not be copied into globally activated guidance.

QC has separate human-feedback and paid-worker admission gates. Disabling paid QC work preserves authorized saved feedback and history. Archiving fences new work; deleting an analysis removes its QC feedback/diagnostics and invalidates dependent case packs and evaluations before removing source evidence. Restoring an archive does not restart cancelled trials.

See the [QC API and worker contracts](server/qc/README.md) and [prompt-versioning reference](docs/prompt-registry.md) for limits, lifecycle guarantees, and integration details.
...
**Re-score with the current rules** (the dialog's recommended choice, policy `full-reassessment-v1`) is for totals withheld by older scoring rules, for example a v3 assessment that left a positively weighted criterion unassessed even though the resume was readable. It runs the complete assessment, evidence-gap review, and independent grounding review again for that comparison against the same frozen resume and rubric, using the models, prompts, and settings current when the request is accepted. Any criterion score can change. The new result is published automatically only after it passes grounding review, and it can still honestly withhold the total when evidence genuinely cannot be assessed. A failed or unreviewed re-score leaves the current result in place. It is published as a separate revision in the same run with the same request, actor, reason, and history audit as a correction; the original result, other comparisons in the run, and their pinned rules are unchanged. A re-score is new model work, so it needs the new-analyses feature and is refused while new work is paused. Re-scored revisions record no QC confidence sidecar and show **Not recorded** in QC. The narrower **Record verified missing evidence as 0 / 5** choice keeps the behavior described above.

Accepted re-scores and corrections also show on the analysis page, not only in the review dialog. While one is queued or running, its comparison's **Status / actions** cell shows **Re-score queued** or **Re-score running** (or the correction equivalent) with the date it was requested, and the progress panel counts them, for example "13 re-scores in progress · 2 queued · 11 running". The row keeps its current result until the work finishes. It then returns to **Complete**, showing the published revision, or the unchanged result if the work failed or was cancelled. Sorting by status treats these rows as in progress. The page keeps checking the comparison list while any work is pending and refreshes the run's score counts once it finishes. Everyone who can open the analysis sees this status, including on the comparison view. The request's reason, requester, criteria, and failure findings remain visible only to owners and editors.
...
Ordinary retries keep the models, prompts, and settings captured when the run was accepted. When a failure came from those older rules, use **Retry with current rules** on a failed or cancelled pair, or on the whole run. The same frozen inputs are then processed with the rules current at the time of the request. The admitted settings stay recorded unchanged on the comparison; a separate `settingsUpgrade` records the adopted rules, when they were requested, and by whom. Later ordinary retries of that comparison keep the adopted rules. Completed comparisons, the run's own pin, and the other comparisons are unchanged. If the current rules are identical, this is an ordinary retry. Like a new analysis, it requires the new-analyses feature and is refused while new work is paused; ordinary retries remain available.

Every structured-output response schema is checked against the Azure OpenAI strict-mode subset before a request is sent. Discriminated unions are sent as `anyOf`, because strict mode rejects `oneOf`; the complete local contract still validates every response, so this cannot admit output the contract would reject. A schema the service would reject fails immediately without a token request or model call, as a non-retryable Score software problem. When the service itself rejects a request (an HTTP 4xx other than 401, 403, or 429), the comparison or correction records a non-retryable `service-unavailable` error that says the request was rejected before the model read it. That is a Score configuration or software problem, not a finding about the documents, and retrying unchanged work will not help. A failed correction still retains the previous result.
...
New completed assessments schedule candidate summaries in the analysis worker. Scoring and summary work take turns within each worker execution, so candidate summaries start while other comparisons are still being scored instead of waiting for the whole scoring queue; freezing inputs and cancellation still go first. Which kind goes first alternates by clock minute, so executions limited to one item still take turns. The **analyses: execution items** Admin setting (default 2, at most 100) bounds how many items each execution claims across both kinds. A job overview waits until that target's comparisons have settled and its completed candidate summaries are ready. Retrying unfinished comparisons marks the affected overview outdated; after the work finishes, it is regenerated using the updated saved results. Summaries use the configured analysis model and frozen evidence, not newer live job or resume versions. Summary failures are reported separately and never turn an already completed assessment into failed scoring.
...
Summary-aware progress and turn-taking between scoring and summaries add no stored field, Admin switch, environment variable, or new resource. Deploy the API/UI and every worker together with `scripts\deploy.ps1`. Workers that predate turn-taking still finish all work, but they generally start summaries only after scoring work runs out.
...
CSV's C1, C2, and subsequent columns follow the saved criterion order for that row's job, with names and weights available in the linked review. Criterion scores use a 0-5 scale. Jobs reuse these columns instead of multiplying spreadsheet width; unused cells are empty. A numeric zero stays zero; `Not assessed` and `N/A` remain distinct. An unavailable overall score has an empty numeric cell and its reason in **Overall assessment**. UTF-8 BOM, quoted multiline fields, and CRLF records support spreadsheet import. Untrusted text beginning with spreadsheet formula markers is prefixed with an apostrophe. Links are plain absolute URLs, not spreadsheet formulas. CSV exports use stable columns and filenames for the selected saved analysis.

PDF/Word contents entries and PowerPoint agenda entries use native internal links to the exact job/grade section, including when titles repeat or sections span several pages/slides. These internal links are distinct from source links. All three formats link each featured candidate to **View analysis**, **View resume**, and **View job** (or **View grade requirements**). Overview names also link to the analysis. These open the saved review and the frozen sources used for that assessment, not newer library versions. Source links require the recipient's workspace access and do not embed credentials or grant access to private documents. Report links open the saved workspace records in the authenticated app.

Reports reuse the selected current saved scores and evidence, including an explicitly reviewed correction revision where one exists; **exporting does not invoke AI, enqueue summaries, or reassess candidates**. PDF, Word, and PowerPoint read full saved narrative and overview text instead of manufacturing summaries during export. CSV retains its existing concise assessment and exact criterion columns. GS qualification concerns remain separate and unscored, and HTML, Markdown, or Word sections are never labeled as printed PDF pages. Scores describe document evidence, not a person's intrinsic ability, hiring suitability, or official GS eligibility.
...
| --- | --- |
| AI task bindings | Eleven independent bindings cover job rubrics, resume profiling, GS competency/draft/review, assessment/review, and candidate/target/reduction/review summaries. A blank deployment override inherits the application default; an unavailable explicit selection never silently falls back. The optional rubric assistant reuses the job-rubric binding rather than adding its own. |
...
| Admission and intake | Feature switches and a new-work pause constrain new operations, formats, URLs, files, batches, reference selections, criteria, and comparison counts; the rubric AI assistant switch is on by default. They cannot override missing services or the verified Word rollout gate. |
...
| Presentation | Application title, theme default, initial page, announcement, help links, and polling defaults do not overwrite saved personal theme preferences or explicit deep links. |
...
Provisioning creates separate job/grade/resume/analysis/QC Cosmos and private Blob stores, dedicated worker identities, and the existing shared Document Intelligence/model/isolated-renderer services. No extra model, Search index, or LinkedIn resource is introduced. The postdeploy hook builds the renderer and one shared worker image containing five independent worker entry points, waits for the renderer's latest revision, and validates the registry, identities, store boundaries, and processing-service configuration.
...
`AZURE_WORKER_CONTAINER_IMAGE` and `AZURE_GRADE_WORKER_CONTAINER_IMAGE` remain independent. New `AZURE_RESUME_WORKER_CONTAINER_IMAGE`, `AZURE_ANALYSIS_WORKER_CONTAINER_IMAGE`, and `AZURE_QC_WORKER_CONTAINER_IMAGE` start with placeholder images, **not** an inherited job/grade image. Their schedules and provisioning-time flags stay disabled until independently saved pins identify the verified shared image and required services exist. Deployment saves each pin only after its matching `dist-worker/resume-worker.mjs`, `dist-worker/analysis-worker.mjs`, or `dist-worker/qc-worker.mjs` entry point executes successfully. Shared packaging also includes all corresponding runtime bundles.
...
The worker workflow builds fresh `score-worker:resume-analysis-word-v1-*` tags. An image tag alone is **not evidence of Word, prompt, QC, or runtime-settings compatibility**. Both web and worker containers require their sibling `word-parser.mjs`; worker packaging writes a versioned `dist-worker/word-imports.json` SHA-256 manifest covering every worker/runtime bundle and the parser. Each manual initial execution validates the manifest and actual artifacts, the extraction runtime's `WORD_EXTRACTION_VERSION=score-word-extraction-v1`, all five runtimes' `RUNTIME_SETTINGS_VERSION=score-runtime-settings-v2` and `PROMPT_RUNTIME_VERSION=score-prompt-runtime-v1`, and the QC runtime's `QC_RUNTIME_VERSION=score-qc-worker-v1` before running its ordinary entry point. A retagged older image, missing reader, or self-consistent manifest with an older runtime cannot pass.

Word admission is enabled only after **all five job, grade, resume, analysis, and QC workers** pass in the same rollout, their execution metadata and current schedules still identify the same verified image/entry points, and no incompatible old executions remain active. Failed or partial verification leaves Word disabled without overwriting unsuccessful workers' independent pins. An ambiguous enablement response triggers a disablement attempt; if Azure cannot confirm that, deployment reports the failure and operators must verify the setting before proceeding. Older in-flight executions must finish before retrying the worker rollout. Do not bypass these checks by manually setting the flag to `true`.

**Runtime settings have their own reader-first gate: `SCORE_RUNTIME_SETTINGS_ENABLED`.** Provisioning writes `false`. Pre-provision, pre-web-deploy, and worker configuration close Word, runtime-settings, and paid-QC admission; renderer-only updates leave them closed. The web application initializes the configuration and prompt stores, and all five workers receive only container-scoped read access. The renderer receives no settings credentials. Admission reopens only after the same five-reader build is verified and incompatible executions have drained. An ambiguous activation response triggers verified disablement. `SCORE_RUNTIME_SETTINGS_WORKER_VERSION`, `SCORE_PROMPT_RUNTIME_WORKER_VERSION`, `SCORE_RUNTIME_SETTINGS_VERIFIED_IMAGE`, and `SCORE_RUNTIME_SETTINGS_VERIFIED_AT` record successful rollout evidence, not continuous worker health.

Settings version 2 adds the dedicated `qcPlan` model task and QC processing/worker policies. Existing version-1 settings and accepted snapshots remain readable without rewriting their hashes or silently adding fields. New QC captures explicitly adopt the compatible QC policy; ordinary retries retain their accepted settings and prompt pins.

**QC activates after the verified prompt-aware rollout.** Provisioning writes `QC_ENABLED=false` and `QC_WORKER_ENABLED=false`. A complete successful deployment enables both only after the runtime-settings gate opens. Closing runtime settings also closes paid QC work, but does not hide saved reviews or clear `QC_ENABLED` in an already enabled application. Configured QC stores remain attached for history and lifecycle cleanup even when new QC admission is off. The QC identity has only its own records/blobs, read-only settings/prompt access, and Foundry inference; it has no source-store, directory, renderer, extraction, or prompt-activation privileges.
...
| Human QC reviews | `QC_ENABLED`; enabled after verified deployment | `QC_RECORDS_CONTAINER=qc-records`, `QC_SOURCE_CONTAINER=qc-sources` |
| QC planning and trials | `QC_WORKER_ENABLED`; requires active verified runtime settings | Same dedicated QC stores; read-only application-settings access |
...
| New Word file admissions | `WORD_DOCUMENT_IMPORTS_ENABLED` | No additional stores; requires a real job or resume service and the verified five-worker rollout |
| Rubric AI assistant | None. Available wherever real jobs and the `RUBRIC_MODEL_*` settings are configured; on by default and switched in Admin settings (`features.rubricAssistant`) | No stores; reuses the `jobRubric` binding; advertised as `rubricAssistant` |

Absent environment flags disable the corresponding gated feature. `/api/features` reports configured availability and authoritative import/analysis limits; its `realAnalyses` field indicates readiness for **new runs**, not the existence of saved history. `markdownJobImports` and `markdownResumeImports` follow the availability of their respective real services without a new global flag. `wordDocumentImports` is true only when the explicit Word flag and at least one real job/resume service are available. Missing or false format-capability fields disable their respective uploads; older APIs lacking all of them remain **PDF-only**. The Word gate controls new Word admissions, never stored source/schema support or Markdown availability. Disabling a source feature does not disable authorized historical analysis operations when the analysis API itself remains enabled.

Cosmos names must be distinct across workspace/job/grade/resume/analysis/QC, application settings, and application access stores, including inactive-feature defaults; content Blob names must likewise be distinct. The API uses `COSMOS_ENDPOINT`, `COSMOS_DATABASE`, and `STORAGE_ACCOUNT_URL`; workers receive only their own store names and dedicated managed identity. The API-only pair `SCORE_ACCESS_CONTAINER=application-access` and `SCORE_ENTRA_SERVICE_PRINCIPAL_ID=<Score-enterprise-app-object-id>` must be configured together. The ID is the Score **service principal object ID**, not its client ID or the API managed identity ID. All workers reuse `RUBRIC_MODEL_ENDPOINT`, `RUBRIC_MODEL_DEPLOYMENT`, `RUBRIC_MODEL_NAME`, and `RUBRIC_MODEL_REASONING_EFFORT`. Only extraction workers receive `DOCUMENT_INTELLIGENCE_ENDPOINT` and internal `JOB_RENDERER_URL`. Execution bounds are `RESUME_WORKER_MAX_ITEMS=5`, `ANALYSIS_WORKER_MAX_ITEMS=2`, and `QC_WORKER_MAX_ITEMS=2`, with a 900-second job timeout, one replica, and no platform retry loop; durable item-level backoff handles transient failures and shared-model throttling.
...
| Cosmos DB | Serverless workspace directory/membership and feature records, including `qc-records`, partitioned by `/workspaceId`; application settings and immutable prompts partitioned by `/applicationId`; API-only `application-access` grants/audit records partitioned by `/tenantId` |
| Blob Storage | Private `workspace-state`, `job-sources`, `grade-sources`, `resume-sources`, `analysis-sources`, `qc-sources`, `documents`, and `knowledge` containers; shared-key access disabled |
...
| Container Apps Jobs | Separate job, grade, resume, analysis, and QC workers, each with its own scoped identity and 1 CPU / 2 GiB; bounded work per execution |
...
For explicit local API development only, set `SCORE_AUTH_MODE=dev-header` and `SCORE_DEV_USER_ROLES` to a JSON map such as `{"<tenant-user-object-id>":["Score.User"],"<other-object-id>":["Score.Admin"]}`. Send `X-Score-Dev-Principal: <configured-tenant-id>:<configured-user-object-id>` from the local Vite proxy. Roles come from that local configuration, never the request header. There is no fallback to a production allowlist; `dev-header` is rejected in production and on App Service.
...
- `GET` and `PUT /api/workspaces/:id/state` return **410 Gone** with `This version of Score is out of date. Reload the page.` and touch no storage, so stale browser tabs fail clearly.
- New workspaces are not seeded with document content; Cosmos directory metadata and membership creation are authoritative.
- Legacy `${workspaceId}/state.json` blobs are ignored and are deleted best-effort when a workspace is permanently deleted. The `workspace-state` Blob container remains because it holds the on-demand per-workspace mutation lease blob `${workspaceId}/mutation.lock`.
- Workspace lifecycle impact counts come only from feature stores. Blockers come only from real lifecycle dependencies such as analyses and seed ladders. Legacy sample runs in old `state.json` blobs no longer block workspace deletion. Archive/unarchive updates directory metadata rather than a state document.
- `features.samplesVisible` is retired: it remains in the strict settings schema, defaults, and types so stored settings revisions, the legacy baseline, and immutable processing snapshots still parse, but it is no longer shown in Application settings and has no effect.
- The legacy `data` query parameter (`real` or `samples`) is ignored, not rejected, so old bookmarks and links in previously exported reports keep working. New internal links and report links omit it.
- There is no data migration. Stored records and server/worker contracts are unchanged: real documents keep `sample: false`, and records keep `dataKind: 'real'`.
...
The workspace-authorized `GET /api/workspaces/:id/summary` endpoint returns lightweight, partition-scoped work counts for members and application Admins without downloading library records or source documents to the home page. Missing or unavailable stores are shown explicitly. Archived workspaces and incomplete workspace lifecycle operations do not display active totals. Counts refresh on returning home, refreshing the directory, or focusing the app; home does not start processing or poll background work.

Cosmos stores directory and membership documents together under the `/workspaceId` partition key. Directory metadata and memberships are authoritative for workspaces. Job, grade, resume, analysis, QC, settings, and access mutations use separate versioned APIs and Cosmos ETags. The `workspace-state` Blob container remains for per-workspace mutation lease blobs and legacy state cleanup, not active workspace content. Concurrent edits produce an explicit conflict instead of silently overwriting another session.

Explicit creation prepares required metadata before atomically publishing directory metadata and the creator's Owner membership. A failed or ambiguous publication does not delete another initializer's work. Ordinary session reads are side-effect-free and never recreate a missing/deleted workspace or seed content.

The browser keeps document state in memory, not browser local storage. Only appearance, whether the sidebar is collapsed, and up to ten recent workspace IDs with last-opened times are remembered locally, with workspace preferences scoped to tenant and user. Home shows up to three available active recent workspaces when there is more than one active workspace. Recency is local to this browser, not synchronized across devices; names, counts, searches, and private documents are not stored with it. An older last-selected ID can seed recents without inventing a last-opened timestamp. Returning home, switching, or signing out protects unsaved editors and waits for acknowledged server writes. Save failures retain the newest edits; conflict recovery is explicit.
...
| Reviewer | Read accessible evidence, submit QC reviews, and curate/evaluate improvement plans; no ordinary editing, source processing, or workspace management |
...
| Application Admin | Owner-equivalent ordinary access to all tenant workspaces; may manage creation grants and settings. QC additionally requires explicit workspace membership; only application Admins can activate evaluated prompt releases |
...
Workspace access is checked server-side, not inferred from visible buttons. The browser refreshes access on focus, directory opening, permission changes, and authorization errors, and stops unauthorized writes on downgrade/removal. Existing accepted work remains workspace-owned: removing its submitter neither cancels others' processing nor deletes results.

QC does not inherit implicit application-admin workspace access. Its APIs require a current explicit membership, and the client clears private QC state when that membership is removed even if ordinary admin access remains. The reviewer-only compatibility API accepts exact Entra object IDs and verifies current application-role eligibility; it never relies on deprecated OID allowlists or changes sign-in admission.
...
| Analysis | Removes the run, comparisons, correction heads/history, narratives, QC reviews/diagnostics, and saved input/evidence snapshots; dependent QC case packs and evaluations are invalidated. |

Workspace lifecycle changes require Owner or application Admin access. Owners and Editors can manage individual items; Readers and Reviewers can search and inspect archived content without changing it. Archiving or deleting the current workspace returns to workspace home rather than silently opening another workspace. You may archive or delete the last active workspace: home offers creation only when authorized and restoration when permitted.
...
The authenticated app reads, uploads, extracts, and stores actual source content for enabled job, grade-reference, resume, analysis, and QC features. Real analyses process frozen source evidence through the configured server and worker stores described above. Public URL ingestion uses bounded server-side fetching rather than the user's browser session.

Theme preference uses `score-theme`, and the collapsed-sidebar preference uses `score-sidebar-collapsed` (`true` or `false`); neither holds workspace data. Recent workspace IDs and view state are local convenience data only; private documents, counts, searches, and saved evidence remain server-owned.
...
| --- | --- |
| `src\app` | Navigation, themes, workspace routing, and server-owned job/grade/resume/analysis state |
...
| `src\services` | Job/grade/resume/analysis API clients, report generation helpers, and browser-side integration utilities |
| `src\features` | Jobs, resumes, rubrics (including the AI-assisted rubric editor), grade-ladder review, and analysis screens; `src\features\assist` holds the reusable assisted-editing framework |
| `src\styles` | Token-based visual system and responsive layouts |
| `server` | Authenticated workspace/job/grade/resume/analysis APIs, the scoped rubric-assistant runner (`server\assist`), authorization, Cosmos adapters, and private Blob storage |
