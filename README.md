...
Correction writes and worker claims default **off**. `ANALYSIS_EVIDENCE_CORRECTIONS_ENABLED=true` enables the API capability `analysisEvidenceCorrections` and the analysis worker; the infrastructure parameter passes the same explicit setting to both. First deploy compatible API/UI, worker, history, and report readers with the flag off, drain older analysis-worker executions, and verify compatibility before enabling it. Turning off new correction admission does not make corrected history unreadable, and rollback must retain correction-aware readers. Run/workspace cancellation, archive, deletion, and expired/superseded leases fence publication; deletion also removes correction heads, proposals, and history. A production rollout and correction of existing records require separate approval and a fresh comparison of the audited result hashes. No historical correction is performed automatically by deploying or opening this code.
