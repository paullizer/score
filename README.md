...
Serialize provisioning and deployments; do not reuse or overwrite fresh image tags. For maintenance, use `node scripts\azure-worker.mjs disable-admission` to close Word and new runtime-policy admissions. The legacy `disable-word` command closes only Word admission. Closing a gate does **not** roll saved permissions back to permissive defaults, cancel accepted operations, or invalidate historical evidence. Keep compatible readers and corresponding real services available; do not roll back to binaries that cannot read already-persisted Word records or policy snapshots. The normal rollout must complete again to reopen admission.
