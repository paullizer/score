...
**QC is a separate cloud-only review workspace, not a required step in ordinary analyses.** Workspace reviewers, editors, owners, and application admins with workspace membership can review saved comparisons. Reviewers cannot import, rescore, edit rubrics, manage sources, or otherwise perform ordinary workspace writes. A workspace owner grants or removes reviewer access using the immutable Entra user ID of a user already allowed to sign in. Granting membership does not invite an external account, expand the deployment sign-in allowlist, or make someone an application admin. Existing original-download and report-export role policies remain independent.
...
| Cosmos DB | Serverless workspace directory/membership and separate `job-records`, `grade-records`, `resume-records`, `analysis-records`, and `qc-records` queues partitioned by `/workspaceId`; application settings and immutable prompts partitioned separately by `/applicationId` |
...
The data model supports `owner`/`editor`/`reviewer`/`viewer` memberships. Owners can manage reviewer access to an existing workspace; other role grants, group creation, and Entra group resolution remain disabled. Reserved `group` workspaces must use these same server-side membership boundaries rather than client-side filters. Application-admin designation does not grant access to another person's workspace.
