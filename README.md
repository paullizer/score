...
6. **Reports:** Open a saved analysis and choose **Export report** for CSV, PDF, Word, or PowerPoint. Export the entire group or one exact job/grade. CSV, PDF, and PowerPoint focus on completed reviews; Word retains the full detailed record, including unfinished comparisons.
...
| PDF | A searchable report with concise job context, an overview of every completed candidate, and featured candidate reviews containing an assessment, scorecard, and brief criterion explanations. |
| Word (`.docx`) | The detailed editable record, including every comparison, full saved assessments, quotations, source locators, and provenance, with native tables, headings, and page numbers. |
| PowerPoint (`.pptx`) | An editable widescreen deck with job context, an overview of every completed candidate, and at most three slides per featured candidate: overview, scorecard, and score explanations. |
...
PowerPoint candidate sections never grow beyond three slides. When a large scorecard cannot fit at a readable text size, the deck shows **Key criteria**, balancing important strengths and gaps, and links to the full scorecard. PDF retains every criterion for featured candidates and can use additional pages for unusually large scorecards. Full evidence and technical records remain available in the app and Word; shortening the other exports does not alter saved results.

Export becomes available after at least one comparison completes, even if its overall score is withheld. Concise reports show **Reporting on X of Y candidates** (or **candidate-job reviews** for multiple targets) and explain unfinished, failed, or cancelled results once, rather than repeating a "Partial report" badge. CSV includes completed assessments only; Word still includes unfinished comparisons as explicit statuses, never zero scores. Status is captured during export, so later completions are not added to that file. Completed results must still be retrieved and verified in full: an access, evidence, or generation failure stops the download. Closing the dialog or leaving the workspace cancels preparation.
...
PDF and PowerPoint link each featured candidate to **View analysis**, **View resume**, and **View job** (or **View grade requirements**). Overview names also link to the analysis. These open the saved review and the frozen sources used for that assessment, not newer library versions. Links require the recipient's workspace access and do not embed credentials or grant access to private documents. Standalone sample links refer to the local app and its saved sample state.

Reports reuse the original saved scores and evidence; **exporting does not invoke AI or reassess candidates**. Concise summaries select relevant saved explanations and surface material gaps, rather than printing generic summary prefixes or inventing favorable claims. Shortened text is visibly marked, with full context available through the links. GS qualification concerns remain separate and unscored, and HTML, Markdown, or Word sections are never labeled as printed PDF pages. Samples remain explicitly fictional. Scores describe document evidence, not a person's intrinsic ability, hiring suitability, or official GS eligibility.
