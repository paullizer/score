...
3. **Rubrics:** Review job-specific criteria or choose **Create grade ladder** from a real job to prepare separate GS levels. Inspect the selected sources and grade matrix before approving supported versions. Sample GS rubrics remain separate. Weights must total 100. When the deployment enables it, **Edit with AI** opens a real job rubric with a scoped assistant beside the job posting. Its changes are applied to your unsaved draft, highlighted until you save, and undoable; see [AI-assisted editing](docs/assisted-editing.md).
