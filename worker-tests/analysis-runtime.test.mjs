...
for (const format of ['docx', 'doc']) {
  for (const targetKind of ['job', 'grade']) {
    test(`${format.toUpperCase()} resume and ${targetKind} seed evidence score only from explicit frozen inputs with stable captured-section citations`, async () => {
      ...
      const narrative = await f.service.summarySubject(f.workspaceId, created.run.id, { kind: 'candidate', subjectId: detail.comparison.id })
      assert.equal(narrative.narrative.status, 'ready')
      assert.equal(narrative.narrative.resultSha256, detail.comparison.result.sha256)
